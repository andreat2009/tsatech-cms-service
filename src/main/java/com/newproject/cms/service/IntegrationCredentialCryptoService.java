...
public class IntegrationCredentialCryptoService {
    ...
    public IntegrationCredentialCryptoService(
        @Value("${cms.config.master-key:}") String explicitMasterKey,
        @Value("${CMS_CONFIG_MASTER_KEY:}") String envMasterKey,
    ...
    public String encrypt(String plainText) {
        ...
        try {
            ...
            throw new IllegalStateException("Unable to encrypt integration credentials", ex);
        }
    }
    ...
    public String decrypt(String encryptedValue) {
        ...
        try {
            ...
            if (parts.length != 2) {
                throw new IllegalStateException("Invalid encrypted integration payload");
            }
            ...
            throw new IllegalStateException("Unable to decrypt integration credentials", ex);
        }
    }
    ...
    private SecretKeySpec resolveKey() {
        try {
            ...
            if (hasText(effectiveMasterKey)) {
                ...
                keyMaterial = digest.digest(("tsa-cms-config|" + effectiveDatasourceUrl + '|' + effectiveDatasourcePassword).getBytes(StandardCharsets.UTF_8));
            } else {
                throw new IllegalStateException("No master key or datasource password available for integration credential encryption");
            }
            return new SecretKeySpec(Arrays.copyOf(keyMaterial, 32), "AES");
        } catch (Exception ex) {
            throw new IllegalStateException("Unable to resolve integration credential encryption key", ex);
        }
    }
